%function bode_asymp(num,den)
clc;clear;close all;
% Put the coefficents of Transfer function
% in the num and den fields, for numerator
...
zroots2=abs(zroots)-1i*real(zroots)./abs(zroots);
proots2=abs(proots)-1i*real(proots)./abs(proots);
...
subplot(2,1,1)
semilogx(interval,y,'bo-','linewidth',1);
hold on; ylabel('20*log(M) (dB)','fontsize',14)
...
%bodemag(sys);
[m1,~,m3]=bode(num,den,{interval(1,1),interval(1,end)});
hold on;
...
yf(indice)=yf_old;
subplot(2,1,2)
semilogx(interval(1,:),yf,'bo-','linewidth',1);
hold on; ylabel('Phase (degree)','fontsize',12)
xlabel('Frequency (rad/sec)','fontsize',12)
set(gca,'fontsize',14)
%plot(interval(1,:),yf,'go');grid on;
[m1,m2,m3]=bode(num,den,{interval(1,1),interval(1,end)});grid on
diff=m2(1)-pp; m2=m2-diff; hold on
semilogx(m3,m2);grid on;drawnow
